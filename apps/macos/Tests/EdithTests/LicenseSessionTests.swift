...
@Suite struct LicenseUpdaterHeaderTests {
    ...
    @Test func freshTokenUsesBearerHeader() {
        let headers = licenseUpdaterHeaders(
            accessToken: StoredAccessToken(token: "tok", expiresAt: iso(offset: 600)), now: now)
    ...
    @Test func expiredTokenYieldsNoHeaders() {
        let headers = licenseUpdaterHeaders(
            accessToken: StoredAccessToken(token: "tok", expiresAt: iso(offset: -600)), now: now)
    ...
    @Test func missingCredentialsYieldNoHeaders() {
        #expect(licenseUpdaterHeaders(accessToken: nil, now: now) == [:])
    }
...
@Suite struct LicenseSessionTests {
    ...
    @Test func activationRequestsChallengeThenSignsIt() async throws {
        ...
        #expect(
            transport.requests[0].url?.absoluteString
                == "https://license.test/api/activation/challenge")
        #expect(
            transport.requests[1].url?.absoluteString == "https://license.test/api/activation")
    ...
    @Test func deactivationClearsLocalStateOnlyAfterServerSuccess() async throws {
        ...
        let store = try storeWithCredentials()
    ...
    @Test func refreshWithoutCredentialThrowsMissingCredentials() async throws {
        ...
        await #expect(throws: LicenseSessionError.missingCredentials) {
            try await session.refresh()
        }
    }

    private func storeWithCredentials() throws -> InMemoryLicenseCredentialStore {
        let store = InMemoryLicenseCredentialStore()
